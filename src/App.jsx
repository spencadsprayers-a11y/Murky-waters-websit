...
export default function App() {
  ...
  const [wafters, setWafters] = useState(
    Object.fromEntries(wafterFlavours.map((flavour) => [flavour, 0]))
  );
  ...
  const sprayTotal =
    Math.floor(sprayItems / 2) * 6 + (sprayItems % 2) * 3.5;

  const wafterTotal =
    Math.floor(wafterItems / 2) * 10 + (wafterItems % 2) * 6;
  ...
  const selectedProducts = [
    ...Object.entries(glooze)
      .filter(([, qty]) => qty > 0)
      .map(([name, qty]) => ({ type: "Fishing Glooze", name, qty })),

    ...Object.entries(sprays)
      .filter(([, qty]) => qty > 0)
      .map(([name, qty]) => ({ type: "25ml Booster Spray", name, qty })),

    ...Object.entries(wafters)
      .filter(([, qty]) => qty > 0)
      .map(([name, qty]) => ({ type: "15mm Barrel Wafters", name, qty })),

    ...Object.entries(pellets)
      .filter(([, qty]) => qty > 0)
      .map(([name, qty]) => ({ type: "Pellets", name, qty })),
  ];

  const selectedText = selectedProducts
    .map((item) => `${item.type} - ${item.name} x${item.qty}`)
    .join("\n");
...
Notes: ${customer.notes || "None"}`;
  ...
  const handleStripeCheckout = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/create-checkout-session", {
        ...
        body: JSON.stringify({
          ...
          totals: {
            ...
            delivery,
            finalTotal,
            discountCode: discountActive ? "MURKYWATERS20" : "",
          },
        }),
      });
      ...
      if (data?.url) {
        ...
        alert("Payment could not be started. Please message Murky Waters on WhatsApp.");
      }
    } catch (error) {
      alert("Payment error. Please try again or send your order on WhatsApp.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-black px-4 pb-44 text-white">
      <section className="relative overflow-hidden rounded-b-[2rem] border-b border-yellow-500/30 bg-gradient-to-b from-yellow-900/40 via-black to-black py-12 text-center">
        ...
        <div className="relative">
          ...
          <p className="mx-auto mt-4 max-w-md text-lg text-gray-300">
            Fishing Glooze, booster sprays, wafters and pellets built for proper
            results on the bank.
          </p>
          ...
          <p className="mt-4 text-sm text-gray-400">
            Liquids and wafters delivery £3.50 • Pellet orders delivery £3.95
          </p>
      ...
      <section className="mt-6 rounded-3xl border border-pink-500/20 bg-zinc-950 p-4">
        ...
        <p className="mb-5 mt-2 text-center text-gray-400">
          £3.50 each or 2 for £6 — for hookbaits only.
        </p>
        ...
        <div className="grid grid-cols-2 gap-4">
          {sprayFlavours.map((item) => (
            <ProductCard
              ...
              desc="Instant hookbait booster spray."
      ...
      <section className="mt-6 rounded-3xl border border-orange-500/20 bg-zinc-950 p-4">
        <h2 className="text-center text-3xl font-black">15mm Barrel Wafters</h2>
        <p className="mb-5 mt-2 text-center text-gray-400">
          £6 per pot or 2 pots for £10 — mix & match any flavours.
        </p>
        ...
        <div className="grid grid-cols-2 gap-4">
          {wafterFlavours.map((item) => (
            <ProductCard
              ...
              price="£6 per pot"
              tag={
                item === "Sweet Mango Black Pepper"
                  ? "Team Favourite"
                  : item === "Squid + Octopus"
                  ? "Big Fish"
                  : ""
              }
              desc="Balanced 15mm barrel wafters."
      ...
      <section className="mt-6 rounded-3xl border border-yellow-500/30 bg-zinc-950 p-5 text-center">
        ...
        <div className="mt-4 space-y-2 text-left text-gray-300">
          <div className="flex justify-between">
            <span>Glooze</span>
            <span>£{gloozeTotal.toFixed(2)}</span>
          </div>

          <div className="flex justify-between">
            <span>Booster sprays</span>
            <span>£{sprayTotal.toFixed(2)}</span>
          </div>

          <div className="flex justify-between">
            <span>Wafters</span>
            <span>£{wafterTotal.toFixed(2)}</span>
          </div>

          <div className="flex justify-between">
            <span>Pellets</span>
            <span>£{pelletTotal.toFixed(2)}</span>
          </div>
        ...
        <div className="space-y-3 text-sm text-gray-300">
          {[
            ...
            [
              ...
              "Yes — Glooze is 3 for £20, sprays are 2 for £6 and wafters are 2 for £10.",
            ],
            [
              "How much is delivery?",
              "Standard liquid and wafter orders are £3.50. If pellets are selected, delivery is £3.95.",
            ],
            [
        ...
        <div className="fixed bottom-0 left-0 right-0 border-t border-yellow-500/20 bg-black/95 p-4">
          <div className="mx-auto grid max-w-md gap-2">
            <button
              ...
              className="rounded-2xl bg-yellow-400 py-5 text-center text-xl font-black text-black shadow-lg disabled:opacity-60"
            >
              {loadingPayment
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="block rounded-2xl bg-green-500 py-4 text-center text-base font-black text-white shadow-lg"
            >
